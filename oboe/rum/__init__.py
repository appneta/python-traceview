...
def header():
    ...
    if not CUSTOMER_RUM_ID or not oboe.SwigContext.isValid():
        # no valid customer UUID found, or not tracing this request: no RUM injection
        return ''
    xid = oboe.SwigContext.toString()
    return r'''(function(){var e=this._tly={q:[],mark:function(a,b){e.q.push(["mark",a,b||(new Date).getTime()])},measure:function(a,b,c){e.q.push(["measure",a,b,c||(new Date).getTime()])},done:function(a){e.q.push(["done",a])},cid:"''' + CUSTOMER_RUM_ID + r'''",xt:"''' + xid + r'''"};e.mark("firstbyte");var f;f=function(){};var g=0;function h(a){return function(b){b[a]||(b[a]=!0,e.measure(["_ajax",b.a,a]))}}var i=h("recv"),j=h("send");
